...
def set_data( c, rword, data):
    ...
    try:
       run_query = query % ('naver',current_day,rword,data.get(1,''),data.get(2,''),data.get(3,''),data.get(4,''),data.get(5,''),data.get(6,''),data.get(7,''),data.get(8,''),data.get(9,''),data.get(10,''),'')
       c.execute( run_query )
...
if __name__ == "__main__":
   ...
   try: 
      ...
      for k in kall[3:13]:
         ...
         #print aurl
         file.write("%s\n"% aurl)
         html = urllib2.urlopen(aurl).read()
         toxml = lh.fromstring(html)
