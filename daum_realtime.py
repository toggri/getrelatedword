...
def set_data_all( c, data):
    ...
    try:
       run_query = query % ("daum",current_day, data.get(1,''), data.get(2,''), data.get(3,''), data.get(4,''), data.get(5,''), data.get(6,''), data.get(7,''), data.get(8,''), data.get(9,''), data.get(10,''))
...
def set_data( c, rank, data):
    ...
    try:
       run_query = query % (to_day,curr_hour,curr_min, rank, data,'daum')
       #file.write(run_query)
       #print "%s" % run_query
       c.execute( run_query )
    except:
...
if __name__ == "__main__":
   ...
   file = codecs.open("/home/tokiman/naver/daum.log", "a", "utf-8")
   file.write(">> %s %s:%s\n" % (to_day, curr_hour, curr_min))
   all_data={}
   try: 
      html = requests.get('http://www.daum.net', headers={'User-Agent': 'Mozilla/5.0 AppleWebKit/531.21.10'}, timeout=30)
      toxml = lh.fromstring(html.text)
      rt_rank = toxml.xpath('//ol[@id="realTimeSearchWord"]/li/div/div[1]/span[@class="txt_issue"]/a')
      rt_tmp = []
      i=0
      for a in rt_rank:
          if i==0:
             rt_tmp.append("%s"%a.xpath('strong/text()')[0].strip())
          else:
             rt_tmp.append("%s"%a.xpath('text()')[0].strip())
          i = i + 1
      rt_search = enumerate(rt_tmp, 1)  
      for a in rt_search:
         #print "%s %s"%(a[0],a[1])
         log="%s %s\n"%(a[0],a[1])
         file.write(log)
